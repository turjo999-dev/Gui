...
public class ShopCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleSellCommand(CommandSender sender, String[] args) {
        ...
        // Route through GuiManager, exactly like the in-shop Quick Sell button does.
        // Previously this constructed a QuickSellGui directly and only registered it in
        // GuiManager's tracking map - but GuiListener read from its own separate map, so
        // clicks inside a GUI opened via /sell (and its aliases /sellgui, /quicksell)
        // were never recognised: selling silently did nothing, and the drag handler's
        // null-guarded branch let fixed buttons be dragged out of the GUI. GuiManager is
        // now the single place that opens this GUI, so every entry point behaves the
        // same way.
