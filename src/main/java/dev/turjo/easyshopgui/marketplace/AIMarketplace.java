...
public class AIMarketplace {
    ...
    private void adjustPricesWithAI() {
        for (Map.Entry<String, MarketData> entry : marketData.entrySet()) {
            ...
            try {
                ...
                double supplyFactor = calculateSupplyFactor(itemId);
    ...
    /**
     * Calculate supply factor from current stock level relative to max stock.
     *
     * Fixed: this used to look up stockData.get(data.toString()) - calling default
     * Object.toString() on the MarketData instance instead of using the itemId string
     * key everything else in this class is keyed by. That could never match a real key,
     * so this always returned 0.0 and stock levels never influenced AI pricing, despite
     * SUPPLY_INFLUENCE being a meaningfully weighted input. Now takes itemId directly so
     * the lookup actually succeeds.
     */
    private double calculateSupplyFactor(String itemId) {
        try {
            StockData stock = stockData.get(itemId);
    ...
    /**
     * Get the raw market data (trend, sentiment, transaction counts) for an item, or
     * null if the AI engine has no data for it yet. Exposed so UI code (MarketplaceGui)
     * can show genuinely computed trend/sentiment/volume rather than hardcoded text.
     */
    ...
    /**
     * Item IDs ranked by total trading activity (buy + sell transaction count),
     * highest first - a real measure of "trending" rather than arbitrary map order.
     * Items with zero recorded activity are excluded.
     */
    public List<String> getTrendingItemIds(int limit) {
        return marketData.entrySet().stream()
                .filter(e -> (e.getValue().getBuyTransactions() + e.getValue().getSellTransactions()) > 0)
                .sorted((a, b) -> Integer.compare(
                        b.getValue().getBuyTransactions() + b.getValue().getSellTransactions(),
                        a.getValue().getBuyTransactions() + a.getValue().getSellTransactions()))
                .limit(limit)
     ...
     */
    /**
     * Market data for an item (price history, trend, sentiment, transaction counts).
     * Public so external classes (e.g. MarketplaceGui) can read real computed values
     * via AIMarketplace.getMarketData() instead of duplicating fake/hardcoded stats.
     */
