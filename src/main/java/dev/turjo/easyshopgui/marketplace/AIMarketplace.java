...
public class AIMarketplace {
    ...
    private void manageStockWithAI() {
        for (Map.Entry<String, MarketData> entry : marketData.entrySet()) {
            ...
            double stockLevel = (double) data.getCurrentStock() / data.getMaxStock();
    ...
    private double calculateTrendFactor(String itemId) {
        PriceHistory history = priceHistory.get(itemId);
        if (history.getPrices().size() < 5) return 0.0;
        
        List<Double> prices = history.getPrices();
        int size = prices.size();
        double recentAvg = prices.subList(size - 3, size).stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
        double olderAvg = prices.subList(size - 6, size - 3).stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
