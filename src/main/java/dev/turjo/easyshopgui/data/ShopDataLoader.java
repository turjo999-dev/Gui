    ...
    private int loadItemsForSection(ShopSection section, ConfigurationSection itemsConfig) {
        ...
        for (String itemId : itemsConfig.getKeys(false)) {
            ...
            try {
                ...
                // Potion type (only meaningful for POTION/SPLASH_POTION/LINGERING_POTION
                // materials, but harmless to parse for any material - createItemStack()
                // only applies it when the built ItemMeta is actually a PotionMeta).
                String potionTypeName = itemConfig.getString("potion-type", null);
                if (potionTypeName != null) {
                    try {
                        item.setPotionType(org.bukkit.potion.PotionType.valueOf(potionTypeName.toUpperCase()));
                    } catch (IllegalArgumentException e) {
                        Logger.warn("Invalid potion-type for item " + itemId + ": " + potionTypeName + ", leaving unset");
                    }
                }
